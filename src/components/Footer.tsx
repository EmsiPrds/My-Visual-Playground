...
export default function Footer({ darkMode: _propDarkMode }: FooterProps) {
  ...
  return (
    <footer
      className={`relative overflow-hidden pt-24 pb-12 transition-colors duration-500 ${isDark
    ...
    >
      ...
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full pointer-events-none opacity-[0.03] select-none overflow-hidden ${isYellow ? "opacity-[0.05]" : ""}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex flex-col">
            <div className="flex items-center space-x-3 mb-8 group cursor-pointer">
              <div className="w-16 h-16 bg-transparent flex items-center justify-center transition-transform duration-300 group-hover:scale-110">
                <img src={logo} alt="MVP Visuals Logo" className="w-full h-full object-contain" />
              </div>
              <span className="font-fugaz text-2xl tracking-tighter uppercase">
                MVP Visuals
              </span>
            </div>
            <p className={`text-sm leading-relaxed mb-8 font-poppins font-light ${isDark ? "text-gray-400" : isYellow ? "text-yellow-800/80" : "text-gray-500"}`}>
              ...
              {quickLinks.map((link) => (
                <li key={link.name}>
                  <button
                    ...
                    className={`group flex items-center text-sm font-medium transition-all duration-300 ${isDark ? "text-gray-400 hover:text-accent" : isYellow ? "text-yellow-800 hover:text-yellow-600" : "text-gray-600 hover:text-accent"
                      }`}
                  >
          ...
          <motion.div variants={itemVariants}>
            ...
            <ul className="space-y-4 text-sm font-poppins font-medium">
              {[
                ...
                >
                  <div className={`w-1.5 h-1.5 rounded-full ${isYellow ? "bg-yellow-400" : "bg-accent/40"}`} />
                  <span>{service}</span>
                </li>
              ))}
            </ul>
          ...
          <motion.div variants={itemVariants}>
            ...
            <p className={`text-sm mb-8 font-poppins ${isDark ? "text-gray-400" : isYellow ? "text-yellow-800/80" : "text-gray-600"}`}>
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.98 }}
              className={`flex items-center p-4 rounded-2xl border-2 transition-all duration-300 group ${isDark
            ...
            >
              ...
              <div className="overflow-hidden">
                <p className={`text-[10px] uppercase font-bold tracking-[0.2em] mb-0.5 ${isYellow ? "text-yellow-600" : "text-accent"}`}>Contact Us</p>
                <p className="text-sm font-bold font-poppins truncate">contact@mvpvisuals.com</p>
              </div>
            </motion.a>
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={{ once: true }}
          className={`pt-12 border-t flex flex-col md:flex-row justify-between items-center gap-8 ${isDark ? "border-white/10" : isYellow ? "border-yellow-200" : "border-gray-100"
            }`}
        >
          <div className="flex flex-col items-center md:items-start gap-2">
            <p className={`text-xs font-poppins tracking-wider ${isDark ? "text-gray-500" : isYellow ? "text-yellow-700" : "text-gray-400"}`}>
              &copy; {new Date().getFullYear()} <span className="text-accent font-bold">MVP VISUALS</span>.
              Designed with &hearts; by our team.
            </p>
          </div>
      ...
      <div className={`absolute -bottom-24 -left-24 w-96 h-96 bg-accent/10 blur-[120px] rounded-full -z-10 animate-pulse ${isYellow ? "bg-yellow-400/20" : ""}`} />
      <div className={`absolute -top-24 -right-24 w-96 h-96 bg-accent/5 blur-[100px] rounded-full -z-10 ${isYellow ? "bg-yellow-500/10" : ""}`} />
    </footer>
  );
}
